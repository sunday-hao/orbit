...
#       bash examples/on_policy_distillation/unified_300step_constant/run-m4-opd-fkl-non-thinking-full.sh
#
...
# === Recipe identity ===
LAUNCHER_NAME=unified_300step_constant_m4_opd_fkl_full_non_thinking_4096
...
if [ "${RUN_EVAL}" = "1" ]; then
    if [ ! -d "${EVALCHEMY_ROOT}/eval/chat_benchmarks" ]; then
        echo "[m4_opd_fkl_full] ERROR: EVALCHEMY_ROOT='${EVALCHEMY_ROOT}' has no eval/chat_benchmarks/" >&2
        exit 1
    fi
fi

...
#       bash examples/on_policy_distillation/unified_300step_constant/run-m4-opd-fkl-non-thinking-lora.sh
#
...
# === Recipe identity ===
LAUNCHER_NAME=unified_300step_constant_m4_opd_fkl_lora_non_thinking
...
if [ "${RUN_EVAL}" = "1" ]; then
    if [ ! -d "${EVALCHEMY_ROOT}/eval/chat_benchmarks" ]; then
        echo "[m4_opd_fkl_lora] ERROR: EVALCHEMY_ROOT='${EVALCHEMY_ROOT}' has no eval/chat_benchmarks/" >&2
        exit 1
    fi
fi
...
if [ "${RUN_TRAIN}" = "1" ]; then
    echo "[m4_opd_fkl_lora] $(date +%FT%T) training start -> ${SAVE_DIR}"
    train_status=0
    ( source "${ORBIT_ROOT}/scripts/lib/launcher.sh" ) || train_status=$?
    if [ "${train_status}" -ne 0 ]; then
        echo "[m4_opd_fkl_lora] training failed (exit ${train_status}); see ${RUN_LOG}" >&2
        exit "${train_status}"
    fi
    echo "[m4_opd_fkl_lora] $(date +%FT%T) training done"
else
    echo "[m4_opd_fkl_lora] skipping training (RUN_TRAIN=0)"
fi

# === Stage 2: evaluate every saved checkpoint ===
if [ "${RUN_EVAL}" != "1" ]; then
    echo "[m4_opd_fkl_lora] skipping evaluation (RUN_EVAL=0)"
    exit 0
fi
if [ ! -d "${SAVE_DIR}" ]; then
    echo "[m4_opd_fkl_lora] ERROR: ${SAVE_DIR} does not exist -- nothing to evaluate." >&2
    exit 1
fi

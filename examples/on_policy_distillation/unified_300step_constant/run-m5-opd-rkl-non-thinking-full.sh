...
#       bash examples/on_policy_distillation/unified_300step_constant/run-m5-opd-rkl-non-thinking-full.sh
#
...
# === Recipe identity ===
LAUNCHER_NAME=unified_300step_constant_m5_opd_rkl_full_non_thinking
...
if [ "${RUN_EVAL}" = "1" ]; then
    if [ ! -d "${EVALCHEMY_ROOT}/eval/chat_benchmarks" ]; then
        echo "[m5_opd_rkl_full] ERROR: EVALCHEMY_ROOT='${EVALCHEMY_ROOT}' has no eval/chat_benchmarks/" >&2
        exit 1
    fi
fi

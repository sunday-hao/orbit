...
#       bash examples/on_policy_distillation/unified_300step_constant/run-m6-opd-st-non-thinking-lora.sh
#
...
# === Recipe identity ===
LAUNCHER_NAME=unified_300step_constant_m6_opd_st_lora_non_thinking
...
if [ "${RUN_EVAL}" = "1" ]; then
    if [ ! -d "${EVALCHEMY_ROOT}/eval/chat_benchmarks" ]; then
        echo "[m6_opd_st_lora] ERROR: EVALCHEMY_ROOT='${EVALCHEMY_ROOT}' has no eval/chat_benchmarks/" >&2
        exit 1
    fi
fi

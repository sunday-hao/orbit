#!/usr/bin/env bash
# M5 -- exact full-vocabulary reverse KL on refreshed student rollouts,
# LoRA.
#
...
#
# Ported from trl/recipes/unified/300step_constant/m5_opd_rkl_non_thinking.yaml.
...
#
# Objective: full-vocab RKL. Identical to M4 except for the KL direction, which is
# the single controlled variable between the two.
#
# Mapping from the recipe: distillation_objective=jsd + beta=1.0 becomes
# --loss-type opd_jsd_loss --opd-jsd-beta 1.0, which orbit documents as reverse
# KL(student||teacher). Both endpoints are hard-coded branches in
# opd_jsd_loss_function, not evaluations of the mixture formula.
#
# Checkpoints are iter_*/adapter. The eval stage merges each into dense HF weights
# with peft before serving.
#
...
#       bash examples/on_policy_distillation/unified_300step_constant/run-m5-opd-rkl-non-thinking-lora.sh
#
...
# === Recipe identity ===
LAUNCHER_NAME=unified_300step_constant_m5_opd_rkl_lora_non_thinking
...
if [ "${RUN_EVAL}" = "1" ]; then
    if [ ! -d "${EVALCHEMY_ROOT}/eval/chat_benchmarks" ]; then
        echo "[m5_opd_rkl_lora] ERROR: EVALCHEMY_ROOT='${EVALCHEMY_ROOT}' has no eval/chat_benchmarks/" >&2
        exit 1
    fi
fi

...
# REINFORCE-style teacher_log_prob - student_log_prob advantage. Sized as a TP smoke test.
set -euo pipefail
...
SAVE_DIR="${ORBIT_ROOT}/orbit_ckpts/check"
...
# === Training schedule ===
# Smoke-test sized: a handful of steps is enough to prove the TP plumbing, and the loss holds
# ~5 live [response_len, vocab/tp] fp32 tensors at once, which is what actually bounds memory
# here (both GPUs also host the teacher and the student rollout under --colocate).
...
# A few steps is all this check needs; override NUM_ROLLOUT to train for real.
...
# than shipping a vocab-sized logprob vector per token over HTTP. --loss-type opd_jsd_loss
# then computes the exact divergence directly from logits -- no advantage/returns pipeline,
# hence --disable-compute-advantages-and-returns.
RL_ARGS=(
...
# TP=2 is the point of this run: the vocabulary splits across both ranks and the divergence
# reduces over them (orbit/backends/training_utils/vocab_parallel.py). --sequence-parallel is
# safe alongside it -- the output layer all-gathers the sequence before the vocab-parallel
# matmul, so each rank still sees the full sequence and only its own vocab shard.
PERF_ARGS=(
...
EVAL_ARGS=(
    # Effectively off: aime24/aime25 at 16 samples x 8192 tokens says nothing about TP and
    # would dominate the wall clock of a 3-step check.
